...
# --- 頁面設定 ---
st.set_page_config(page_title="台股 AI 戰情室 V12.0", layout="wide", page_icon="🦅")

# --- CSS 優化 (修復 Dark Mode 問題) ---
st.markdown("""
<style>
    /* 強制設定文字顏色，解決 Dark Mode 看不見的問題 */
    .analysis-box { 
...
STOCK_DB = {
    ...
    "📱 光電/其他": {
        "3008.TW": "大立光", "3406.TW": "玉晶光", "2409.TW": "友達", "3481.TW": "群創", "6116.TW": "彩晶", "2327.TW": "國巨", "2492.TW": "華新科", "2428.TW": "興勤", "6209.TW": "今國光", "4915.TW": "致伸", "4938.TW": "和碩", "2353.TW": "宏碁", "2324.TW": "仁寶", "3293.TWO": "鈊象", "3532.TW": "台勝科", "6409.TW": "旭隼", "2395.TW": "研華", "2059.TW": "川湖", "3533.TW": "嘉澤", "2912.TW": "統一超", "5903.TW": "全家", "1216.TW": "統一", "8454.TW": "富邦媒", "2412.TW": "中華電", "3045.TW": "台灣大", "4904.TW": "遠傳"
    }
}
...
def get_advanced_fundamentals(ticker):
    try:
        ...
        profit_margin = info.get('profitMargins', None) # 淨利率
        ...
        if ocf is not None and ocf < 0: risks.append("🔴 營業現金流為負 (燒錢)")
        if gross_margin and gross_margin < 0.1: risks.append("🟠 毛利率過低 (<10%)")
        debt = info.get('debtToEquity', 0)
        if debt and debt > 150: risks.append("⚠️ 負債比過高 (>150%)")

        return {
            "本益比(預估)": round(forward_pe, 2) if forward_pe else "N/A",
            "本益比(歷史)": round(trailing_pe, 2) if trailing_pe else "N/A",
            "PEG": round(peg_ratio, 2) if peg_ratio else "N/A",
            "PB(股價淨值比)": round(pb_ratio, 2) if pb_ratio else "N/A",
            "EPS(過去)": round(trailing_eps, 2) if trailing_eps else "N/A",
            "EPS(預估)": round(forward_eps, 2) if forward_eps else "N/A",
            "ROE": f"{round(roe*100, 2)}%" if roe else "N/A",
            "淨利率": f"{round(profit_margin*100, 2)}%" if profit_margin else "N/A",
...
def analyze_stock(ticker, strict_mode=False):
    try:
        ...
        bb_status = "一般"
        if latest['BB_Width'] < 0.15: bb_status = "⚡ 壓縮蓄勢"; score += 5
        elif price > latest['BB_High'] and vol_ratio > 1.2: bb_status = "🚀 突破噴出"; score += 15; signals.append("突破布林")
        ...
        return {
            ...
            "籌碼狀態": chip_status, "布林型態": bb_status,
...
def generate_summary(data):
    ...
    elif "噴出" in bb: summary += " \n\n🔥 **警示**：股價帶量突破布林上緣，屬於強勢攻擊訊號。"
    return summary

# --- 主程式介面 ---
st.sidebar.header("🦅 掃描設定 (V12.0)")
selected_sectors = st.sidebar.multiselect("選擇板塊", list(STOCK_DB.keys()), default=["🔥 熱門 ETF (Top 30)", "💻 半導體/AI"])
strict_mode = st.sidebar.checkbox("嚴格篩選模式", value=False)

st.title("🦅 台股 AI 戰情室 V12.0")
st.caption("新增：獲利能力分析 (ROE/淨利率)、估值指標 (PEG/PB)、DarkMode 修復")
...
# --- Tabs ---
tab1, tab2 = st.tabs(["📋 掃描排行榜", "🔬 個股深度透視"])
            ...
            if data:
                ...
                with ct:
                    ...
                    if fund_data:
                        ...
                        f1.metric("ROE (股東權益報酬)", fund_data['ROE'])
                        f2.metric("淨利率", fund_data['淨利率'])
                        f3.metric("過去EPS", fund_data['EPS(過去)'])
                        f4.metric("預估EPS", fund_data['EPS(預估)'])
                        
                        # V12 新增：估值指標
                        v1, v2, v3, v4 = st.columns(4)
                        v1.metric("預估本益比", fund_data['本益比(預估)'])
                        v2.metric("PEG (成長估值)", fund_data['PEG'])
                        v3.metric("股價淨值比", fund_data['PB(股價淨值比)'])
                        v4.metric("歷史本益比", fund_data['本益比(歷史)'])
                ...
                with cc: st.plotly_chart(plot_chart(data), use_container_width=True)
